...
#define SDI12SENSOR_COMPANY "COMPNAME"  // 8 Characters depicting company name
#endif
...
#define SDI12SENSOR_OTHER_INFO \
  "001"  // (optional) up to 13 char for serial or other sensor info
#endif
...
#define CMD_PARAM_SIGN_BIT 5U  // Parameter value is signed '+' or '-'
...
typedef struct SDI12CommandSet_s {
  ...
  int16_t param1 =
    0;  // Storage of primary command parameter i.e aM1~9 or new address aAb!
  int16_t param2 =
    0;  // Storage of secondary command parameter, i.e Identify meta group aI_001~999!
...
class SDI12Sensor {
 ...
 private:
  ...
  static SDI12Sensor*
    last_set_active_object_;  // Reference to the last set active SDI12Sensor object
 ...
 public:
  ...
  static SDI12Sensor*
  LastActive(void);  // Get current last set active SDI12Sensor object
  static void
  ClearLastActive(void);  // Clears the reference to last set active SDI12Sensor object
  static bool
       IsSetLastActive(void);  // Check if last set active SDI12Sensor instance is set
  void SetCrcRequest(const bool crc_request);
  bool CrcRequested(void) const;
  static const SDI12CommandSet_s ParseCommand(const char* received,
                                              const char  ack_address = '\0',
                                              char**      _endptr     = NULL);
 ...
 protected:
  ...
  static bool RuleIsAddressChange(const SDI12SensorCommand_e cmd, const int param1,
                                  const uint8_t flags);
  static bool RuleIsMeasurement(const SDI12SensorCommand_e cmd, int param1,
                                const uint8_t flags);
  static bool RuleIsConcurrent(const SDI12SensorCommand_e cmd, int param1,
                               const uint8_t flags);
  static bool RuleIsContinuous(const SDI12SensorCommand_e cmd, const int param1,
                               const uint8_t flags);
  ...
  static bool RuleIsHighVolumeMeasure(const SDI12SensorCommand_e cmd,
                                      const uint8_t              flags);
  static bool RuleIsIdentifyGroup(const SDI12SensorCommand_e cmd1,
